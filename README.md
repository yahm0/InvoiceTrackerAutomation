...
4. Duplicate detection uses Message ID, so no duplicates will appear as long as the same emails are in Gmail.
...
2. Check `SPREADSHEET_ID`, `FOLDER_ID`, `RECIPIENT_EMAIL` and replace all `YOUR_*` placeholders with real values.
...
4. Re-run and check the Logs sheet for `extractTextViaVision_` entries to confirm it fired.
...
3. The next run will not re-add them because duplicate detection checks both Invoice Number and Message ID.
